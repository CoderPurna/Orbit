...
import { recordingStatusEnum, pipelineStatusEnum, contentVisibilityEnum, actionStatusEnum } from "./enums";
...
export const recording = pgTable("recording", {
  ...
  startedByParticipantId: uuid("started_by_participant_id").references(() => meetingParticipant.id, {
    onDelete: "set null",
  }),
...
export const transcript = pgTable("transcript", {
  ...
  durationSeconds: integer("duration_seconds"),
  r2Key: text("r2_key"),
  visibility: contentVisibilityEnum("visibility").notNull().default("host_only"),
...
export const transcriptSegment = pgTable("transcript_segment", {
  ...
  participantId: uuid("participant_id").references(() => meetingParticipant.id, { onDelete: "set null" }),
...
export const meetingSummary = pgTable("meeting_summary", {
  ...
  visibility: contentVisibilityEnum("visibility").notNull().default("host_only"),
  editedByUserId: text("edited_by_user_id").references(() => user.id, { onDelete: "set null" }),
  attempts: smallint("attempts").notNull().default(0),
  lastError: text("last_error"),
...
export const actionItem = pgTable("action_item", {
  ...
  assigneeParticipantId: uuid("assignee_participant_id").references(() => meetingParticipant.id, {
    onDelete: "set null",
  }),
