import { pgTable, text, timestamp, varchar, smallint, numeric, jsonb, bigint, uuid, unique } from "drizzle-orm/pg-core";
...
export const usageLedger = pgTable("usage_ledger", {
  ...
  sessionId: uuid("session_id").references(() => meetingSession.id, { onDelete: "set null" }),
...
export const usageDailyRollup = pgTable(
  "usage_daily_rollup",
  {
    ...
    totalCostUsd: numeric("total_cost_usd", { precision: 12, scale: 6 }).notNull(),
  },
...
export const auditLog = pgTable("audit_log", {
  ...
  actorParticipantId: uuid("actor_participant_id").references(() => meetingParticipant.id, {
    onDelete: "set null",
  }),
...
export const webhookEvent = pgTable(
  ...
  (table) => [unique("idx_webhook_external").on(table.provider, table.externalEventId)]
);
